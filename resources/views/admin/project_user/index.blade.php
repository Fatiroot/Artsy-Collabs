    ...
    <div class="w-full flex flex-col h-screen overflow-y-hidden">
        ...
        <header class="w-full items-center bg-white py-2 px-6 hidden sm:flex">
            ...
            <div x-data="{ isOpen: false }" class="relative w-1/2 flex justify-end">
                <a href="dashboard"
                    class="text-darck text-3xl font-semibold uppercase hover:text-gray-300">{{ Auth::user()->name }}</a>
                        ...
                        <table class="min-w-full bg-white">
                            <thead class=" text-white">
                                <tr>
                                    ...
                                    <th class="w-1/5 text-left py-3 px-4 uppercase font-semibold text-sm">Project Name
                                    </th>
                                    <th class="w-1/5 text-left py-3 px-4 uppercase font-semibold text-sm">Artist</th>
                            ...
                            <tbody class="text-gray-700">
                                <tr>
                                    <td class="w-1/5 text-left py-3 px-4">
                                        <img src="{{$projectuser->user->getFirstMediaUrl('images')}}" alt=""
                                            class="rounded-full h-12 w-12">
                                    </td>
                                    <td class="w-1/5 text-left py-3 px-4"> @if($projectuser->project)
                                        {{$projectuser->project->title}}
                                        @else
                                        ------
                                        @endif</td>
                                    <td class="w-1/5 text-left py-3 px-4"><a class="hover:text-blue-500"
                                            href="tel:622322662" value="">

                                            <span>{{$projectuser->user->name}},</span>

                                        </a></td>
                                    <td class="w-1/5 text-left py-3 px-4">
                                        <form action="{{ route('project-user.update', $projectuser->id) }}"
                                            method="POST">
                                            @csrf
                                            @method('PUT')
                                            <button type="submit" class="hover:text-blue-500" value="">
                                                <p class="text-gray-900 whitespace-no-wrap">
                                                    @if($projectuser->status===0)
                                                    <span
                                                        class="bg-green-100 text-green-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-green-900 dark:text-indigo-300">accepted</span>

                                                    @else
                                                    <span
                                                        class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded dark:bg-red-900 dark:text-indigo-300">pending</span>
                                    ...
                                    <td class="w-1/5 text-left py-3 px-4">
                                        <form action="{{ route('project-user.destroy', $projectuser->id) }}"
                                            method="post">
                                            @method('DELETE')
                                            @csrf
                                            <button type="submit" style="background: none; border: none;">
                                                <i class="fas fa-trash" style="color: #e8b04f;"></i>
                                            </button>
                                        </form>
                                    </td>
                                </tr>
